...
class Peer {
  ...
  async setupClient(client) {
    ...
    console.log(`Setting up client at ${client.peerAddress}...`);
  ...
  async broadcastPeers() {
    ...
    await this.broadcast(peersMessage);
  ...
  async sendPeersTo(connection, since=this.peersSince_) {
    ...
    await this.sendTo(connection, peersMessage);
  }
  ...
  onPeersMessage(message) {
    ...
    this.discover(peers)
      .then(() => {
        console.log('Done discovering peers from onPeersMessage.');
      })
      .catch(err => {
