...
class Peer {
  constructor({
    ...
    // -------------------------------------------------------------------------
    // Begin Optional / Configurable Parameters --------------------------------
    discoveryConfig     = {
                            addresses: [],
                            range: {
                              start: 26780,
    ...
    logger              = console,
    wsServerConfig      = {},
    // End Optional / Configurable Parameters ----------------------------------
    // -------------------------------------------------------------------------
  }) {
    // -------------------------------------------------------------------------
    // Begin Private Class Properties Assigned Via Constructor Params ----------
    ...
    // End Private Class Properties Assigned Via Constructor Params ------------
    // -------------------------------------------------------------------------

    // -------------------------------------------------------------------------
    // Being Private Class Properties Statically Assigned ----------------------
    this.eventEmitter_ = new EventEmitter();
    this.discoveryAddressBook_ = [];
    this.discoveryOperation_ = null;
    this.initializationOperation_ = null;
    this.isDiscovering_ = false;
    this.isReady_ = false;
    this.isInitializing_ = true;
    this.peers_ = [];
    this.requestHandlers_ = {};
    // End Private Class Properties Statically Assigned ------------------------
    // -------------------------------------------------------------------------
  ...
  async discover(addresses = []) {
     ...
     * time in the past, -1 minutes threshold.
     */
    addresses = 
      addresses.filter((address) => 
        !this.discoveryAddressBook_.hasOwnProperty(address) || 
          (Date.now() - this.discoveryAddressBook_[address] > 60000));
      ...
      Promise.all(
        discoverAddresses
          .map((address) => {
            ...
            return this.discoverPeer(address)
              .catch((err) => {
  ...
  onClientMessage(connection, type, message) {
    ...
    const handlersToInvoke = 
      this.requestHandlers_[type].filter(
        handler => handler.matches(connection.address));
