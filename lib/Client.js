...
class Client {
	...
	onMessage(message) {
		...
		const handlerHashId = 
			crypto.createHash('sha256').update(messageType).digest('hex');
		const handler = this.requestHandlers_[handlerHashId];

	    if(handler) {
	    	this.logger_.log(`Using generic handler: '${handlerHashId}'`);
			...
			if(!this.isTrusted && (isHeloMessage || isSetupCipherMessage) || 
				this.isTrusted && (!isHeloMessage || isSetupCipherMessage)) {
				handler.invoke(messageObj, this);
			} else if(!this.isTrusted && !isHeloMessage) {
				this.logger_.error(
					`Received message but connection is not yet trusted!`);
			}
	    } else {
	    	this.logger_.error(`No handler registered for ${messageType}.`);
	    }
	}

	bind(RequestClass) {
	    if(!RequestClass || !(RequestClass instanceof Function)) {
			throw new Error(`Invalid parameter: 'RequestClass'.`);
	    }

	    const requestHandler = new RequestHandler(RequestClass);
	    const hash = 
	    	crypto.createHash('sha256').update(requestHandler.id).digest('hex');

	    this.requestHandlers_[hash] = requestHandler;
	...
	unbind(RequestClass) {
	    ...
	    const hash = 
	    	crypto.createHash('sha256').update(RequestClass.name).digest('hex');

	    if(this.requestHandlers_.hasOwnProperty(hash)) {
			delete this.requestHandlers_[hash];
			return true;
	    }
				...
				if(err) {
					...
					if(backoff > MAX_MESSAGE_SEND_TIMEOUT_MS) {
						return reject(new Error(
					  		`Max timeout readched when attempting to send message!`));
					}
	...
	get receiveHeloPromise() {
		...
		if(!this.receiveHeloPromise_) {
			this.receiveHeloPromise_ = new Promise((resolve, reject) => {
					const heloHandler = (message, connection) => {
							/*
							 ...
							 * the ring private).
							 */
							const peerPublicKeyBuffer = 
								Buffer.from(message.publicKey, 'utf8');
							const peerSignature = 
								Buffer.from(message.signature, 'hex');
							const peerRsaKeyPair = 
								new RSAKeyPair({
									publicKeyBuffer: peerPublicKeyBuffer });
							...
							const ownSignatureHex = 
								this.credentials_.signature.toString('hex');
							const remoteSignatureHex = 
								peerSignature.toString('hex');

							if(ownSignatureHex === remoteSignatureHex) {
								return reject(new Error(
									`Received signature matching own ` +
									`signature from peer. Closing connection ` +
									`so as to prevent potential connection ` +
									`to self.\n` +
							...
							try {
								const keyIsSigned = 
									this.ringRsaKeyPair_.verify(
										message.publicKey, 
										peerSignature);
								if(!keyIsSigned) {
									throw new Error(
										`Key not signed by same ring private.`);
								}
							} catch(e) {
								return reject(e);
							}
							...
							this.unbind(HeloMessage);
							resolve();
						};

					this.bind(HeloMessage).to(heloHandler);
	...
	get receiveSetupCipherPromise() {
		...
		if(!this.receiveSetupCipherPromise_) {
			this.receiveSetupCipherPromise_ = new Promise((resolve, reject) => {
					const setupCipherHandler = (message, connection) => {
							try {
								const iv = 
									this.credentials_.rsaKeyPair.decrypt(
								...
								return reject(new Error(
									`Could not decrypt AES-256-CBC ` + 
									`properties from SetupCipher message!`));
							}
							this.unbind(SetupCipherMessage);
							return resolve();
						};
					this.bind(SetupCipherMessage).to(setupCipherHandler);
