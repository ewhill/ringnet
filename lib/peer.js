...
module.exports = class Peer extends EventEmitter {
  ...
  discover() {
    ...
    let discoverOne = () => {
      ...
      var parsedAddress = url.parse(peerToDiscover.address);
        ...
        if(self.range && Array.isArray(self.range) && self.range.length == 2
          && self.range[0] <= self.range[1]) {
            ...
            for(let i=self.range[0]; i<=self.range[1]; i++) {
              ...
              let d = { 'address': '${parsedAddress.href',
                'signature': peerToDiscover.signature };
      ...
      let address = peerToDiscover.address.slice(0,5) == 'wss://' ? 
        peerToDiscover.address : 'wss://' + peerToDiscover.address;
