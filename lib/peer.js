...
module.exports = class Peer extends EventEmitter {
  constructor({
    ...
    }, { // We require a valid peer privateKey to join
    ...
    if(!this.noServer) {
      // If we're not provided a valid 'httpsServer' option, check to make sure we are at 
      ...
      if(!this.httpsServer || getClassName(this.httpsServer) !== "Server") {
        /* istanbul ignore if */
        if(this.debug) {
          console.log("A valid 'httpsServer' option was not given; the 'credentials' " + 
            "option will be checked for valid HTTPS credentials instead.");
        }
        
        checks = checks.concat([{
          // We require a valid signature to join and to validate peers on the network
          description: "HTTPS Server Key",
          location:  credentials.hasOwnProperty("key") ? credentials.key : "https.key.pem"
        }, {
          // We require a valid signature to join and to validate peers on the network
          description: "HTTPS Server Certificate",
          location: credentials.hasOwnProperty("cert") ? credentials.cert : "https.cert.pem"
        }]); 
      } else {
        /* istanbul ignore if */
        if(this.debug) {
      ...
      /* instanbul ignore if */
      if(this.debug) {
        console.log("Option 'noServer' set to true; SKIPPING creation of HTTPS server.");
      }
    }
    ...
    // Peep the addresses variable for valid, given discovery addresses, adding them to 
    // this.discoveryAddresses as we go...
    ...
    this.privateKey = new NodeRSA(fs.readFileSync(this.privateKeyLocation));
      
    // If peer public key file exists, then read it. Else, generate public from this.privateKey
    this.publicKeyLocation = publicKey;
    this.publicKey = new NodeRSA((fs.existsSync(this.publicKeyLocation) ? 
      fs.readFileSync(this.publicKeyLocation) : this.privateKey.exportKey("public")));
    
    // Read the signature file (this is required to exist)
    this.signatureLocation = signature;
    this.signature = fs.readFileSync(this.signatureLocation);
    ...
    */
    ...
    // Only create the https server if we haven't specified the `noServer` property in 
    // `this.wsServerOptions`.
    /* istanbul ignore else */
    if(!this.noServer) {
  ...
  discover() {
    ...
    let enqueue = (address) => {
      // Ensure address is not already in queue, not already connected to, not this peer
      ...
      if(!inDiscoveryAddresses && !isConnectedTo && !isOwnSignature) {
        // If we haven't seen this discovery address before and we aren't already connected 
        // to it, push it to our discovery queue
        /* istanbul ignore if */
        if(this.debug) console.log(`\t${JSON.stringify(address)}`);
    ...
    let discoverOne = () => {
      ...
      // next() will be called when we're done discovering on a single address and can move 
      // on to the next.
      let next = () => {
      ...
      if(this.isConnectedTo(peerToDiscover) || this.isOwnSignature(peerToDiscover.signature)) {
        next();
      }
      ...
      let [ , protocol, host, port, path, query, hash ] 
        = urlRegex.exec(peerToDiscover.address);
      ...
      if(hasPort) {
        try {
          ...
          /* istanbul ignore if */
          if(this.debug) {
            console.log(`Error parsing port of address to discover!`);
          }
      ...
      /*
        The NodeJS URL library is very strange... It's string output, when 'format' is
      ...
      */
      if(!hasPort) {
        ...
        /*
          ...
          Since our address was incomplete, return to prevent connecting to it.
        */
        return next();
      }
  ...
  setupConnection({ connection, request=null }) {
    ...
    */
    ...
    connection.on('close', (code) => {
      ...
      if(code !== 1000) {
        if(connection.hasOwnProperty("originalAddress")) {
          ...
          /*
            On an abnormal close, we should try to discover in order to reattempt 
            connection with the peer we lost
          */
    ...
    /*
      ...
      NOTE: this handshake happens BOTH ways - e.g. a HELO is responded to by a HELO of 
      our own, making the handshake in total.
    */
    try {
      /*
        We have to send our public key and public key signature (signed by the ring.pem) 
        to the connection (peer) for validation. The peer will do the same for us, so we 
      ...
      */
      ...
      /*
        In case of error, log the stack. Most likely, if we're here, it is the result 
        of an export error in NodeRSA (above) or a message send error (connection.send).
      */
  ...
  broadcast(options) {
    ...
    let { message=msg, 
      connection=false, 
      confirmationBackoff=5000 } = options || {};
      
    // If there are no peers to broadcast to, exit
    if(this.peers.length < 1) {
      /* istanbul ignore if */
      if(this.debug) console.error(`ERROR: No peers to broadcast message to!`);
      return;
    }
    ...
    if(!message || typeof message == 'undefined') {
      /* istanbul ignore if */
      if(this.debug) console.error(`ERROR: Message missing or not valid!`);
      return;
    }
    
    /* istanbul ignore if */
    if(this.debug) console.log(`Broadcasting ${message} to ${this.peers.length} peers...`);
    ...
    for(let p of toSendTo) {
      if(p.connection.connected) {
        if(p.connection.trusted) {
          ...
          /*
            If the 'requireConfirmation' flag is set on this peer, then we need to check back at 
            a scheduled timeout as to whether the message has been received by the peers we sent 
            the message to.
          */
          if(this.requireConfirmation && 
            messageCopyToSend.header.type !== Message.TYPES._confirm) {
            /*
              Create timeout to check if the mssage was confirmed by the remote peer. If so, we 
              simply step out of the timeout function, but if not then we try to resend the 
              message to the remote peer. Wrap in anonymous function in order to preserve scope.
            */
            ((self, peer, msg) => {
              self.managedTimeouts.setTimeout(() => {
                for(let i=peer.connection.confirmedMessages.length-1; i>=0; i--) {
                  ...
                  if(peer.connection.confirmedMessages[i].header.hash == msg.header.hash
                    && peer.connection.confirmedMessages[i].header.timestamp.toString() == 
                    msg.header.timestamp) {
                      const found = peer.connection.confirmedMessages.splice(i,1)[0];
          ...
          try {
            ...
            messageCopyToSend.header.signature = 
              (this.privateKey.sign(JSON.stringify(messageCopyToSend.body)))
                .toString('base64');
            ...
            messageCopyToSend.body = Buffer.concat([cipher.update(messageBodyBuffer), 
            ...
            // Something went wrong with the encryption, most likely, so let's gracefully 
            // fail and exit...
            /* istanbul ignore if */
            if(this.debug) {
              console.error("ERROR: broadcast to TRUSTED connection failed. This " + 
                "could be and more likely is due to an encryption error. Exiting now.");
              console.error(e.stack);
            }
          }
        } else {
  ...
  getPeerList(signaturesToOmit) {
    ...
    /*
      Add list of our known peers to the body, so that, when received by the other peer, it can
      discover those addresses as well, creating a fully connected, bidirectional graph (network).
    */
    for(let i=0; i<this.peers.length; i++) {
      if(this.peers[i].connection.hasOwnProperty("peerPublicKeySignature") && 
