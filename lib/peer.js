...
module.exports = class Peer extends EventEmitter {
  ...
  setupConnection({ connection, request=null }) {
    ...
    connection.on('error', (err) => {
      ...
      connection.client.connect(connection.parsedAddress);
    });
  ...
  receive({ connection, message }) {
    ...
    if(message.header.type == PeerMessage.PEER_MESSAGE_TYPES.helo) {
      ...
      if(message.header.type == PeerMessage.PEER_MESSAGE_TYPES.trusted) {
        ...
        if(message.body.hasOwnProperty("iv") && message.body.hasOwnProperty("key")) {
          ...
          if(message.body.hasOwnProperty("requireConfirmation") && 
            typeof message.body.requireConfirmation == "boolean") {
              if(this.debug && message.body.requireConfirmation) {
                console.log(`Peer at ${connection.remoteAddress} - ` +
                  `${connection.originalAddress} is requesting message confirmation.`);
              }
        ...
        if(message.hasOwnProperty("header") && message.header.hasOwnProperty("confirmation")) {
          if(this.debug) {
            console.log(`\tPeer would like to confirm receipt of message with ` +
              `hash ${message.header.confirmation}`);
          }
          ...
          for(let i=connection.unconfirmedMessages.length-1; i>=0; i--) {
            ...
            if(connection.unconfirmedMessages[i].header.hash == message.header.confirmation) {
              ...
              if(this.debug)
                console.log(`\tMessage with hash ${message.header.confirmation} has been confirmed.`);
        ...
        try {
          ...
          if(connection.peerPublicKey.verify(decryptedMessageBody, messageSignature)) {
            ...
            if(connection.requireConfirmation) {
              ...
              confirmationMsg.header.confirmation = message.header.hash;
              this.broadcast({
              ...
              (function(self, peer, msg) {
                setTimeout(function() {
                  ...
                  if(found) {
                    ...
                    console.log(`Message has already been confirmed and will not be resent.`);
                  }
                }, 30000);
              })(this, p, messageCopyToSend);
          }
          
          p.connection.unconfirmedMessages.push(messageCopyToSend);
