...
module.exports = class Peer extends EventEmitter {
  ...
  constructor({
    ...
    self.noServer = (self.wsServerOptions.hasOwnProperty("noServer") && 
      typeof self.wsServerOptions == "boolean") ? self.wsServerOptions.noServer : false;
  ...
  discover() {
    ...
    let discoverOne = () => {
      ...
      let hasPath = protocol !== undefined;
      ...
      let client = new WebSocketClient(url.format(parsedAddress));
  ...
  setupConnection({ connection, request=null }) {
    ...
    connection.on('message', function incoming(data) {
      ...
      self.receive({ connection: this, 'message': data });
    });
    ...
    try {
      ...
      var heloCallback = function(err) {
        if(err) { // If at first we don't succeed ...
          setTimeout(function() {
            // ... try, try again
            connection.send(helo.toString(), heloCallback);
          }, 5000);
        }
      };
    ...
    if(message.header.type == PeerMessage.PEER_MESSAGE_TYPES.helo) {
      ...
      if(peerPublicKey && peerPublicKeySignature && keyIsSigned) {
          ...
          var trustedCallback = function(err) {
            if(err) {
              setTimeout(function() {
                connection.send(trusted.toString(), trustedCallback);
              }, 5000);
            }
          };
        ...
        if(p.connection.trusted) {
          ...
          */
          ...
          try {
            ...
            var self = this;
            p.connection.send(JSON.stringify(messageCopyToSend), (err) => sendCallback.apply(self, err));
          } catch(e) {
          ...
          if(message.header.type == PeerMessage.PEER_MESSAGE_TYPES.helo) {
            var heloCallback = function(err) {
              if(err) {
                setTimeout(function() {
                  connection.send(message, heloCallback);
                }, 5000);
              }  
            };
            
            p.connection.send(message, heloCallback);
          } else if(this.debug) {
