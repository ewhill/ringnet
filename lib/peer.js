...
module.exports = class Peer extends EventEmitter {
  constructor({
    ...
    // Peep the addresses variable for valid, given discovery addresses, 
    // adding them to this.discoveryAddresses as we go...
    ...
    // Read the peer private RSA key (required to exist)
    ...
    */
    ...
    /* istanbul ignore else */
    if(!this.noServer) {
      // We weren't given an 'httpsServer' via constructor arguments. We may 
      // need to create one.
      /* istanbul ignore else */
      if(!this.httpsServer || getClassName(this.httpsServer) !== "Server") {
  ...
  enqueueDiscoveryAddress(address)  {
  ...
  discover() {
    ...
    let discoverOne = () => {
      ...
      // next() will be called when we're done discovering on a single address 
      ...
      if(this.isConnectedTo(peerToDiscover) || 
        this.isOwnSignature(peerToDiscover.signature)) {
          return discoverNext();
      }
      ...
      */
      ...
      /*
        If 'parsedAddress' doesn't contain a port and this peer has a given 
        discovery range, 'this.range', let's expand the address into multiple 
        entries in 'this.discoveryAddresses' for the ports specified in 
        'this.range'.
      */
      if(!hasPort) {
        // Check if we were provided a range of ports for discovery
        if(this.range && Array.isArray(this.range) && 
          this.range.length == 2 && typeof this.range[0] === 'number' && 
          typeof this.range[0] === 'number' && this.range[0] <= this.range[1]) {
            // Enqueue address with ports in 'this.range' for discovery
  ...
  setupConnection({ connection, request=null }) {
    ...
    /*
      ...
      and we are able to verify the connection's (peer's) public key via a 
      'trusted' message exchange. Until the said is complete, the connection 
      cannot and will not be trusted and no other messages will be sent / 
    ...
    */
    ...
    /*
      ...
      NOTE: this handshake happens BOTH ways - e.g. a HELO is responded to by a 
      HELO of our own, making the handshake in total.
    */
    try {
      /*
        We have to send our public key and public key signature (signed by the 
        ring.pem) to the connection (peer) for validation. The peer will do the 
        same for us, so we can establish trust with one another.
      */
  ...
  receive({ connection, message }) {
    ...
    /*
      ...
      coming from. Once found, update the 'active' property to the current time 
      (in ms) to reflect when the last message was received from the peer.
    */
