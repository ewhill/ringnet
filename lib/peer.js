...
module.exports = class Peer extends EventEmitter {
  ...
  constructor({
    ...
    self.on('message', ({ message, connection }) => {
      if (message.header.type == PeerMessage.PEER_MESSAGE_TYPES.peers) {
        ...
        // Send the message
        connection.sendUTF(peers.toString(), function(err) {
          if(err) {
            // Do nothing.
          }
        });
  ...
  setupConnection({ connection, request=null }) {
    ...
    if(!connection.hasOwnProperty("originalAddress")) {
      if(request && (request.hasOwnProperty("headers") || request.hasOwnProperty("connection"))) {
        connection.originalAddress = request.headers['x-forwarded-for'] || request.connection.remoteAddress;
        console.log(`Address parsed from request: ${connection.originalAddress}`);
      } else {
    ...
    if(message.header.type == PeerMessage.PEER_MESSAGE_TYPES.helo) {
      ...
      try {
        ...
        if(this.debug) {
          ...
          console.log("\t\t-> Signature: " + peerPublicKeySignature.toString("base64"));
        }
        
        // Verify the peer's public key...
        // TODO: Is this valid and SECURE logic? Sending signature over the wire?
      ...
      if(message.header.type == PeerMessage.PEER_MESSAGE_TYPES.trusted) {
        ...
        if(message.body.hasOwnProperty("iv") && message.body.hasOwnProperty("key")) {
          ...
          if(message.body.hasOwnProperty("listening")) {
            let toParse = message.body.listening;
            
            if(typeof message.body.listening !== "number") {
              try {
