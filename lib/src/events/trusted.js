...
module.exports = function({ connection, message }) {
  ...
  if(message.body.hasOwnProperty("iv") && message.body.hasOwnProperty("key")) {
    ...
    try {
      connection.peerIv = 
        this.peerRSAKeyPair.decrypt(Buffer.from(message.body.iv, 'base64'));

      connection.peerKey = 
        this.peerRSAKeyPair.decrypt(Buffer.from(message.body.key, 'base64'));
      
      this.emit('connection', { connection });
    } catch(e) {
      /* istanbul ignore if */
      if(this.debug) {
        console.error("ERROR: 'trusted' message received but our private " + 
          "key could not decrypt its contents. Exiting now.");
      }
      ...
      typeof message.body.requireConfirmation == "boolean") {
        ...
        if(this.debug) {
          console.log(`Peer at ${connection.remoteAddress} - ` +
            `${connection.originalAddress} on port ` + 
            `${connection._socket.remotePort} ` +
        ...
        if(message.body.listening.hasOwnProperty("port")) {
          ...
          if(typeof message.body.listening.port !== "number") {
            try {
              ...
              if(isNaN(toParse)) throw new Error("Port is NaN!");
    ...
    if(message.body.hasOwnProperty("peers") && 
      Array.isArray(message.body.peers)) {
        // Create variable to compare to the length of discoveryAddresses later.
        let lengthBefore = this.discoveryAddresses.length;
        
        for(let i=0; i<message.body.peers.length; i++) {
          // If leading '::ffff:', we have IPv4 address and can strip it.
          if(message.body.peers[i].address.indexOf("::ffff:") === 0) {
            message.body.peers[i].address = 
              message.body.peers[i].address.slice(7);
          }
          
          /*
           * If we haven't seen a peer in the list of peers that this peer has 
           * given us, then add said peer to our discoveryAddresses array for 
           * discovery at a later time.
           */
          if(!this.inDiscoveryAddresses(message.body.peers[i]) && 
            !this.isConnectedTo(message.body.peers[i]) && 
            !this.isOwnSignature(message.body.peers[i].signature)) {
              /* istanbul ignore if */
              if(this.debug) {
