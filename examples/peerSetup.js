...
A small script to generate necessary RSA Keys / signature for use in a ringnet.

REMEMBER: peers on the same ringnet MUST have a valid signature (signed public 
key) from the same ring private key. If the signature cannot be validated, the 
peer will not be able to join the ringnet. This being said, if peer1 has been 
signed by ring.pem and peer2 wishes to talk to peer1 on peer1's ringnet, then 
peer2 has to have a signature (signed public key) provided from the same ring 
...
// Grab the necessary arguments from `process.argv` using `ArgumentsParser`
const argumentsParser = new ArgumentsParser({
    // --ring=paths<array<str>> [OPTIONAL]
    "ring": ArgumentsParser.ARGUMENT_TYPE_ENUM.STRING_ARRAY,
    // --peer=paths<array<str>> [OPTIONAL]
    "peer": ArgumentsParser.ARGUMENT_TYPE_ENUM.STRING_ARRAY,
    // --o=prefix<str> [OPTIONAL]
    "o": ArgumentsParser.ARGUMENT_TYPE_ENUM.STRING,
    // --out=prefix<str> [OPTIONAL]
    "out": ArgumentsParser.ARGUMENT_TYPE_ENUM.STRING,
    // --ro=prefix<str> [OPTIONAL]
    "ro": ArgumentsParser.ARGUMENT_TYPE_ENUM.STRING,
    // --ringout=prefix<str> [OPTIONAL]
    "ringout": ArgumentsParser.ARGUMENT_TYPE_ENUM.STRING,
    // --b=size<int> [OPTIONAL]
    "b": ArgumentsParser.ARGUMENT_TYPE_ENUM.INT,
    // --h [OPTIONAL]
    "h": ArgumentsParser.ARGUMENT_TYPE_ENUM.BOOL,
    // --help [OPTIONAL]
    "help": ArgumentsParser.ARGUMENT_TYPE_ENUM.BOOL,
    // --s [OPTIONAL]
    "s": ArgumentsParser.ARGUMENT_TYPE_ENUM.BOOL,
    // --silent [OPTIONAL]
    "silent": ArgumentsParser.ARGUMENT_TYPE_ENUM.BOOL,
  });
...
let fileName = args.o || args.out || "",
  ringFileName = args.ro || args.ringout || "",
...
/*
 * Create `debug` function, if `verbose`, from console.log (`debug` only 
 * `console.log`'s if `verbose` is true).
 */
